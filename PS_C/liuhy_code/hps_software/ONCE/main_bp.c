...
#define SHIFT_MEM_IDX_TO_ADDR 13 //here for hps memory cell is 8bits 
...
#define SIZE_TCPIP_SEND_BUF_TRUNK (8192) // 4096 data points, 8192 bytes for one data-sending operation via tcp/ip
...
/*             case ST_COMM_FPGA:
            ...
            case ST_STREAM_MEM:
                ...
                addr_mem_read = (uint8_t*)vaddr_base_buffer + (idx_mem_read << SHIFT_MEM_IDX_TO_ADDR);
                // memcpy((void *)buf_data,addr_mem_read,SIZE_TCPIP_SEND_BUF_TRUNK);

                //send via tcpip , one block (8196) bytes each packet.
                send(s, addr_mem_read, SIZE_TCPIP_SEND_BUF_TRUNK*2, 0) ;
				printf("read fpga memory data is %d\n", (*(uint8_t*)addr_mem_read));
