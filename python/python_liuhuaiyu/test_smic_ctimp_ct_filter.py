...
'''        
HOST = '192.168.2.89'  # Standard loopback interface address (localhost)
...
while (True):
    ...
    list_time_plot2 = list([a/fs for a in range(4095)])
    adc_data_to_filter=array_adc_data_rec6[0:8191]
    b,a = signal.butter(8,0.0125,'lowpass') # 16k low pass   8-order
    adc_data_filtered = signal.filtfilt(b, a, adc_data_to_filter)
    VPP_data=((max(adc_data_filtered[1000:5095])-min(adc_data_filtered[1000:5095]))/12)*1000
    #list_freq_plot = list([(a+1)*fs/num_frames_read for a in range(int(float(num_frames_read)/2))])
    axTime1.plot(list_time_plot2,\
                             adc_data_filtered[1000:5095],c=colors[0],label="Time Domain")
    ...
    axTime2.set_ylabel('VCON',fontsize=20)
    axTime3.set_ylabel('LC_PATTERN',fontsize=20)
