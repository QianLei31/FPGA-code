...
input should be grounded when test IRN.
@author: huaiyu
"""
...
MASK_XOR=0b11110000
MASK_CNT=0b00001111
MASK_SIGN_GEN=0b00000100
MASK_SIGN=0b00000010
MASK_REC_FLAG=0b00000001
MASK_VCON=0b00001000
MASK_VCOP=0b00010000
MASK_MEMO_ADDRESS=0b01111000
MASK_FLAG=0b10000000
...
#    fft_data=fft_data/num_fftavg/(len_fft)/fs
    ...
    dc_power=fres*sum(fft_data[psd_window:round(300/fres)])
...
strPathSave = desdir +"/" +"test_smic_lc"
...
while (True):
    ...
    with open(str_file_read, mode='rb') as h_file_results:
        ...
        array_adc_data_rec5=[]
        array_adc_data_rec6=[]
        array_adc_data_memo_address=[]
        data = h_file_results.read(BYTES_FILE)          
        adc_data_rec = [] #recovered adc data
        for idx_bytes in range(0,len(data),2):
            adc_data_memo_address=((data[idx_bytes+1] & MASK_MEMO_ADDRESS)>>3)
            adc_data_rec1=((data[idx_bytes+1] & MASK_SIGN_GEN)>>2)
            adc_data_rec2=((data[idx_bytes+1] & MASK_SIGN)>>1)
            adc_data_rec3=(data[idx_bytes+1] & MASK_REC_FLAG)
            adc_data_rec4=(data[idx_bytes]& MASK_CNT)
            adc_data_rec5=(data[idx_bytes]& MASK_XOR)>>4
            adc_data_rec6=((2*(adc_data_rec4*16+adc_data_rec5))/(16*15))-1
            ...
            array_adc_data_memo_address.append(adc_data_memo_address)              
        h_file_results.close()
    ...
    #plt.close('all')
    #plt.clf()
    plt.suptitle('Level-Crossing-Assisted VCO-based CTDSM Test',fontsize=30)
    ...
    axTime4.plot(list_time_plot,\
                             array_adc_data_memo_address,c=colors[4],label="memo_address")    
    ...
    #plt.show()
    sndr_result_text='SNDR = '+format(sndr_db,'0.2f')+'dB  ENOB = '+format(enob,'0.2f')+'b  IRN(300-10k) = '+format(IRN*10**6,'0.2f')+'uV'
    
    # axTime1.set_title('Time Domain Waveform',fontsize=20)
    # axTime1.set_xlabel('Time',fontsize=20)
    axTime1.set_ylabel('SIGN_GEN',fontsize=20)
    axTime2.set_ylabel('SIGN',fontsize=20)
    axTime3.set_ylabel('REC_FLAG',fontsize=20)
    axTime4.set_ylabel('CNT_DATA',fontsize=20)
    axTime5.set_ylabel('XOR_DATA',fontsize=20)
    ...
    dest_file_name=strPathSave+ "/" + "ADC_DATA_SAVED_" + str(save_data_index) + ".bin"
