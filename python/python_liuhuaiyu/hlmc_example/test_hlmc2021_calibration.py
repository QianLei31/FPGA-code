...
strPathSave = desdir +"/" +"test_calibration"
...
list_kk=[]
list_sndr=[]
for kk in np.arange(1.36,1.4,0.0001):
    ...
    with open(str_file_read, mode='rb') as h_file_results:
        ...
        for idx_bytes in range(0,len(data),2):
            adc_data_rec=(((data[idx_bytes+1] & MASK_CNT))*2 -13)/15 \
                          ...
                          - (data[idx_bytes] & MASK_LSB_N))*kk)/225)  
    ...
    axTime = plt.subplot(2, 2, 1)
    axFreq = plt.subplot(2, 2, 2)
    axSNDR = plt.subplot(2, 2, 3)
    fs = 2560000
    ...
    list_freq_plot,fft_recording,sndr_db,enob=fft_clean(array_adc_data_rec,fs,2,4062.5,8)
    axFreq.plot(list_freq_plot,\
                             fft_recording,c=colors[1],label="FFT")  
    list_kk.append(kk)
    list_sndr.append(sndr_db)
    axSNDR.plot(list_kk,list_sndr,'-r^')    
    #plt.show()
