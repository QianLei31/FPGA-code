...
#    fft_data=fft_data/num_fftavg/(len_fft)/fs
    ...
    sndr=np.sqrt(sig_power/(sigband_power-sig_power))
...
while (True):
    ...
    axTime = plt.subplot(1, 2, 1)
    axFreq = plt.subplot(1, 2, 2)
    fs = 2560000
    ...
    list_freq_plot,fft_recording,sndr_db,enob=fft_clean(array_adc_data_rec,fs,1,4375,8)
    ...
    axTime.set_title('Time Domain Waveform',fontsize=20)
    axTime.set_xlabel('Time',fontsize=20)
    axTime.set_ylabel('Recovered Data',fontsize=20)
