...
while (True):
    ...
    list_freq_plot,fft_recording,sndr_db,enob,K,IRN=fft_clean(array_adc_data_rec6,fs,1,289,8)
    axFreq.plot(list_freq_plot,\
                              fft_recording,c=colors[6],label="FFT")
    #plt.show()
    sndr_result_text='SNDR = '+format(sndr_db,'0.2f')+'dB  ENOB = '+format(enob,'0.2f')+'b  IRN(0.5-10k) = '+format(IRN*10**6,'0.2f')+'uV'
    ...
    dest_file_name=strPathSave+ "/" + "ADC_DATA_SAVED_" + str(save_data_index)+ "_"+format(IRN*10**6,'0.2f')+ "uV_IRN_1kBW_10kCHOP"+ ".bin"
