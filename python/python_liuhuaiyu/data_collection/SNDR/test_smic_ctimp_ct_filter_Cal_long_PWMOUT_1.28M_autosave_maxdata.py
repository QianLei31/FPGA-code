...
MASK_UP=0b11110000
MASK_CAL_STATE=0b00111100
MASK_CHIP_RST=0b00000010
MASK_DN=0b00001111
...
#    fft_data=fft_data/num_fftavg/(len_fft)/fs
    ...
    sigband_power=fres*sum(fft_data[1:sigband_bins])
    sig_bin=1+round(sin_freq/fres)
    sig_bin_min=max(1,sig_bin-psd_window)
    sig_bin_max=sig_bin+psd_window
    harmonic2_bin=1+round(2*sin_freq/fres)
    harmonic3_bin=1+round(3*sin_freq/fres)
    sig_power=fres*sum(fft_data[sig_bin_min:sig_bin_max])
    harmonic2_power=fres*sum(fft_data[harmonic2_bin-psd_window:harmonic2_bin+psd_window])
    harmonic3_power=fres*sum(fft_data[harmonic3_bin-psd_window:harmonic3_bin+psd_window])
    #K=((0.12**2)/8)/sig_power
    #K=1.3324451240096678e-08
    K=2.1958594496688068e-08
    dc_power=fres*sum(fft_data[1:round(3/fres)])
    IRN_power=sigband_power-sig_power-dc_power-harmonic2_power-harmonic3_power
    ...
    snr=np.sqrt(sig_power/(sigband_power-sig_power-harmonic2_power-harmonic3_power-dc_power))
    snr_db=20*np.log10(snr)
    sfdr=np.sqrt(sig_power/(harmonic2_power))
    sfdr_db=20*np.log10(sfdr)
    enob=(sndr_db-1.76)/6.02
    fft_data=fft_data*K
    return (fft_freq.tolist(),fft_data.tolist(),sndr_db,enob,K,IRN,snr_db,sfdr_db)
...
while (True):
    ...
    with open(str_file_read, mode='rb') as h_file_results:
        ...
        array_adc_data_rec9=[] #PWM_REG_OUT
        array_adc_data_rec10=[] #PWM_REG_IN
        data = h_file_results.read(BYTES_FILE)          
        adc_data_rec = [] #recovered adc data
        for idx_bytes in range(0,len(data),4):
            ...
            adc_data_rec7=((data[idx_bytes+1] & MASK_CAL_STATE)>>2)
            adc_data_rec8=((data[idx_bytes+1] & MASK_CHIP_RST)>>1)
            adc_data_rec10=((data[idx_bytes+2]&(0b01111111))<<2)+(data[idx_bytes+1]>>6)
            adc_data_rec9=(data[idx_bytes+3]<<1)+(data[idx_bytes+2]>>7)
            ...
            array_adc_data_rec9.append(adc_data_rec9)
            array_adc_data_rec10.append(adc_data_rec10)                
        h_file_results.close()

    plt.clf()
    plt.suptitle('Impedance Boosted VCO-based CTDSM Test',fontsize=30)
    #(axTime, axFreq) = plt.subplots(1, 2)
    axTime1 = plt.subplot(5, 2, 1)
    axTime2 = plt.subplot(5, 2, 3)
    axTime3 = plt.subplot(5, 2, 5)
    axTime7 = plt.subplot(5, 2, 7)
    axTime4 = plt.subplot(5, 2, 2)
    axTime5 = plt.subplot(5, 2, 4)
    axTime6 = plt.subplot(5, 2, 6)
    axFreq = plt.subplot(5, 2, 8)
    axTime9 = plt.subplot(5, 2, 9)
    axTime10 = plt.subplot(5, 2, 10)
    #fs = 1280000
    fs=50000000/39
    ...
    # adc_data_to_filter=array_adc_data_rec6
    # sos = signal.butter(8,400,'lowpass',fs=1280000,output='sos') # 16k low pass   2-order
    # adc_data_filtered = signal.sosfilt(sos, adc_data_to_filter)
    # VPP_data=((max(adc_data_filtered[128000:NUM_DATA_POINTS_READ-128000])-min(adc_data_filtered[128000:NUM_DATA_POINTS_READ-128000]))/12)*1000
    # #average for 5 times
    # VPP_avg=(VPP_data5+VPP_data4+VPP_data3+VPP_data2+VPP_data)/5
    # VPP_data5=VPP_data4
    # VPP_data4=VPP_data3
    # VPP_data3=VPP_data2
    # VPP_data2=VPP_data
    ...
    #                          adc_data_filtered,c=colors[0],label="Time Domain")
    ...
    axTime9.plot(list_time_plot,\
                             array_adc_data_rec9,c=colors[0],label="Time Domain")
    axTime10.plot(list_time_plot,\
                             array_adc_data_rec10,c=colors[0],label="Time Domain")
    list_freq_plot,fft_recording,sndr_db,enob,K,IRN,snr_db,sfdr_db=fft_clean(array_adc_data_rec6,fs,1,29.955,8)
    axFreq.plot(list_freq_plot,\
                              fft_recording,c=colors[6],label="FFT")
    #plt.show()
    sndr_result_text='SNDR = '+format(sndr_db,'0.2f')+'dB  ENOB = '+format(enob,'0.2f')+'b  IRN(5-10k)= '+format(IRN*10**6,'0.2f')+'uV'+' SNR = '+format(snr_db,'0.2f')+'dB'+' SFDR = '+format(sfdr_db,'0.2f')+'dB'
    #amplitude_result_text='Vpp = '+format(VPP_data,'0.2f')+'mV,Vpp_avg5 ='+format(VPP_avg,'0.2f')+'mV'
   ...
   #axTime1.set_ylabel('LOW PASSED DATA',fontsize=20)
    ...
    axTime9.set_ylabel('PWM_OUT',fontsize=20)
    axTime10.set_ylabel('PWM_IN',fontsize=20)
    ...
    if sndr_db >= (max_SNDR+0.01):
        dest_file_name=strPathSave+ "/" + "DATA_" + str(save_data_index)+ "_"+format(sndr_db,'0.2f')+ "_SNDR_" + format(snr_db,'0.2f') + "_SNR_"+ format(sfdr_db,'0.2f')+"_SFDR"+".bin"
        shutil.copyfile(str_file_write,dest_file_name)
        save_data_index=save_data_index+1
        print('Successfully save the data as' + dest_file_name + '\n')
        if sndr_db > max_SNDR:
            max_SNDR=sndr_db;
        else:
            max_SNDR=max_SNDR;
    else:
        print('Continue reading data......')
    plt.clf()
