...
NUM_DATA_POINTS_READ=4096*2048 #2.5Hz
...
#    fft_data=fft_data/num_fftavg/(len_fft)/fs
    ...
    K1=2.1958594496688068e-08 #this K is from IRN test
    K2=((0.4**2)/8)/sig_power #input amplitude should be added here for CMRR PSRR test
    XXRR_db=10*np.log10(K2/K1)
    return (fft_freq.tolist(),fft_data.tolist(),sndr_db,enob,XXRR_db)
...
while (True):
    ...
    list_freq_plot,fft_recording,sndr_db,enob,XXRR_db=fft_clean(array_adc_data_rec6,fs,1,9.934,8)
    axFreq.plot(list_freq_plot,\
                              fft_recording,c=colors[6],label="FFT")
    #plt.show()
    sndr_result_text='SNDR = '+format(sndr_db,'0.2f')+'dB  ENOB = '+format(enob,'0.2f')+'  XXRR = '+format(XXRR_db,'0.2f')
    ...
    if XXRR_db >= (max_XXRR+0.01):
        dest_file_name=strPathSave+ "/" + "DATA_" + str(save_data_index)+ "_"+format(XXRR_db,'0.2f')+ "_PSRR"+".bin"
