...
#    fft_data=fft_data/num_fftavg/(len_fft)/fs
    ...
    K2=((0.2**2)/8)/sig_power #input amplitude should be added here for CMRR PSRR test
...
while (True):
    ...
    list_freq_plot,fft_recording,sndr_db,enob,XXRR_db=fft_clean(array_adc_data_rec6,fs,1,1000,8)
    ...
    dest_file_name=strPathSave+ "/" + "ADC_DATA_SAVED_CMRR_" + str(save_data_index) + "_"+str(format(XXRR_db,'0.2f'))+".bin"
    str_input=input('Do you want to save the data as' + dest_file_name + '\n')
    if str_input[0] == 'y':
