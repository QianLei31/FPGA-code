...
#    fft_data=fft_data/num_fftavg/(len_fft)/fs
    ...
    #K=((0.02**2)/8)/sig_power
    K=2.477256573182498e-08
    VPP_CAL=np.sqrt(K*sig_power*8)*1000 #calculated input amplitude mV
    return (fft_freq.tolist(),fft_data.tolist(),sndr_db,enob,K,VPP_CAL)
...
while (True):
    ...
    list_freq_plot,fft_recording,sndr_db,enob,K,VPP_CAL=fft_clean(array_adc_data_rec6,fs,1,200,6)
    axFreq.plot(list_freq_plot,\
                              fft_recording,c=colors[6],label="FFT")
    #plt.show()
    #average for 5 times
    # VPP_avg=(VPP_data3+VPP_data2+VPP_CAL)/3
    # VPP_data3=VPP_data2
    # VPP_data2=VPP_CAL
    
    sndr_result_text='VPP='+format(VPP_CAL,'0.2f')+'mV'
    ...
    str_input=input('Do you want to save the data' + '\n')
    if str_input[0] == 'y':
        str_input_freq=input('Please input the frequency \n')
        str_input_amp=input('Please input the amplitude \n')
        dest_file_name=strPathSave+ "/" + "ADC_DATA_SAVED_" + str(save_data_index)+ "_inputfreq="+str_input_freq+"Hz_"+"_inputamp="+str_input_amp+"mV_outputamp="+format(VPP_CAL,'0.2f')+ "mV"+ ".bin"
        shutil.copyfile(str_file_write,dest_file_name)
        save_data_index=save_data_index+1
        print('Data saved as'+ dest_file_name + '\n')
    else:
