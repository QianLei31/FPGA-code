...
while (True):
    ...
    #axTime6 = plt.subplot(4, 1, 2)
    #axFreq = plt.subplot(5, 2, 8)
    #axTime9 = plt.subplot(4, 1, 3)
    axTime10 = plt.subplot(2, 1, 2)
    fs=50000000/39
    ...
    sos = signal.butter(12,40,'lowpass',fs=50000000/39,output='sos') # 16k low pass   2-order
    adc_data_filtered = signal.sosfilt(sos, adc_data_to_filter)
    
    adc_data_filtered=adc_data_filtered*75
    VPP_data=((max(adc_data_filtered[640000:-1])-min(adc_data_filtered[640000:-1])))


    #list_freq_plot = list([(a+1)*fs/num_frames_read for a in range(int(float(num_frames_read)/2))])
    axTime1.plot(list_time_plot[640000:-1],\
                             adc_data_filtered[640000:-1],c=colors[0],label="Time Domain")
    ...
    #axTime6.plot(list_time_plot[640000:1280000],\
                             #array_adc_data_rec6[640000:1280000],c=colors[5],label="Time Domain")
    ...
    if str_input[0] == 'y':
        dest_file_name=strPathSave+ "/" + "DATA_ECG_IMPB_" + str(save_data_index)+".bin"
    ...
    time.sleep(8)
    print('Waiting finished,start reading data......')
