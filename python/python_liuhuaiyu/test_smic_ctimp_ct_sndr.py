...
# do multiple times and get average fft results
...
while (True):
    ...
    while cnt_read_mem < (NUM_DATA_POINTS_READ * BYTES_DATA_POINTS / SIZE_TCPIP_SEND_BUF_TRUNK):
        ...
        if (len(data) != len_remain_data):
            print(
                "Generated data file has data length of{:d} ".format(len(data)))
    ...
    fs = 2560000
    colors = ['b', 'g', 'r', 'c', 'm', 'xkcd:neon green',
              'k', 'xkcd:tan', 'xkcd:grey', 'xkcd:peach']
    ...
    list_freq_plot, fft_recording, sndr_db, enob = fft_clean(
        array_adc_data_rec6, fs, 1, 2890.625, 8)
    axFreq.plot(list_freq_plot,
                fft_recording, c=colors[6], label="FFT")
    # plt.show()
    sndr_result_text = 'SNDR = ' + \
        format(sndr_db, '0.2f')+'dB  ENOB = '+format(enob, '0.2f')

    # axTime1.set_title('Time Domain Waveform',fontsize=20)
    # axTime1.set_xlabel('Time',fontsize=20)
    axTime1.set_ylabel('VCOP', fontsize=20)
